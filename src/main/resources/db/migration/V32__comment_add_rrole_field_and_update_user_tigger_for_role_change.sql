...
UPDATE comment c INNER JOIN user u on c.user_id = u.id SET c.r_user_role = u.role;
...
CREATE DEFINER=`7bnew`@`localhost` TRIGGER user_AFTER_UPDATE AFTER UPDATE ON user FOR EACH ROW
  BEGIN
    ...
    IF(NEW.role != OLD.role) THEN
      ...
      UPDATE comment SET r_user_role = NEW.role WHERE !NEW.is_deleted and user_id = NEW.id;
    END IF;
  END$$
DELIMITER ;